...
void verificarProductoMatrices(){
    int orden, repeticiones = 1, intentos = 0;
    ...
    time.start();
    bool correcto = verificarProductoRepeticiones(repeticiones, intentos, orden, A, B, C);
    time.stop();
    ...
    mostrarSolucionMontecarlo(correcto, intentos, probabilidad, tiempo_producto, tiempo_verificacion);
}
...
void verificarFallar(){
    ...
    for (int i = 0; i < ejecuciones; i++){
        if(!verificarProductoRepeticiones(repeticiones, intentos, orden, A, B, C)){
            aciertos++;
        }
    }
